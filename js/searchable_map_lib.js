...
var SearchableMapLib = {
  ...
  createSQL: function() {
    ...
    if (name_search != '') {
      SearchableMapLib.currentResults.features = $.grep(SearchableMapLib.currentResults.features, function(r) {
          return r.properties["categories"].toLowerCase().indexOf(name_search.toLowerCase()) > -1;
        });
    }
    ...
    // var tempWhereClause = [];
    // if ( $("#cbType1").is(':checked')) tempWhereClause.push("Pets");
    // if ( $("#cbType2").is(':checked')) tempWhereClause.push("Parents");
    // if ( $("#cbType3").is(':checked')) tempWhereClause.push("Children");
    // if ( $("#cbType4").is(':checked')) tempWhereClause.push("Other");
    // self.whereClause += " AND " + type_column + " IN ('" + tempWhereClause.join("','") + "')";

    var type_column = "'categories'";

    if ( $("#rbType1").is(':checked')) self.whereClause += " AND " + type_column + "=Pets";
    if ( $("#rbType2").is(':checked')) self.whereClause += " AND " + type_column + "=Parents";
    if ( $("#rbType3").is(':checked')) self.whereClause += " AND " + type_column + "=Children";
    if ( $("#rbType4").is(':checked')) self.whereClause += " AND " + type_column + "=Other";
    ...
    SearchableMapLib.currentResultsLayer = L.geoJSON(SearchableMapLib.currentResults, {
        pointToLayer: function (feature, latlng) {
          return L.marker(latlng, {icon: SearchableMapLib.getIcon(feature.properties["categories"], feature.properties["inperson_n_h"])} );
        },
  ...
  // -----custom functions-----
  getIcon: function(type, hybrid){
    // if (type == "Pharmacy") return redIcon;
    // if (type == "Government") return blueIcon;
    // if (type == "POther") return yellowIcon;
    if (hybrid == "y") return GGIconHybrid;
    return GGIcon;
  },
